...
DROP TABLE IF EXISTS `#__foxcontact_domain_blacklist`;
...
WHERE `name` = 'com_foxcontact';

-- Installed extension
DELETE FROM `#__extensions`
WHERE `element` = 'com_foxcontact';

-- Administrator menu item and Site menu items
DELETE FROM `#__menu`
WHERE `link` LIKE '%com_foxcontact%';

-- Joomla auto updater
DELETE FROM `#__update_sites`
WHERE `name` LIKE '%foxcontact%';
