...
FoxHtmlElem::create('div')->attr('id', $current->getBoxId())->classes('fox-item fox-item-dropdown control-group')->classes($current->get('classes'))->classes($board->getItemDecorationClass($current->get('unique_id')))->append(FoxFormRender::render('label'))->append(FoxHtmlElem::create('div')->classes('controls')->append(FoxHtmlElem::create('select')->attr('id', $current->getItemId())->attr('name', $current->getInputName())->attr('style', "{$current->getStyleWidth()}{$current->getStyleHeight()}")->attr('data-placeholder', $current->getPlaceholder())->classes('fox-item-dropdown-select-single')->append(FoxHtmlElem::create('option')->attr('value')->attr('label', ' '))->appends($current->get('options'), function (FoxDesignBase $source) use($current, $value)
{
	return FoxHtmlElem::create('option')->attr('value', $source->get('text'))->text($source->get('text'))->selected($source->get('text') === $value);
